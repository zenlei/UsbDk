...
`Tools\Installer\buildmsi.bat` accepts only `ARM64` for this branch. It also
copies the WDK-generated `UsbDk.cer` beside each MSI when a test certificate is
available. The certificate is a sidecar for VM testing; the installer never
silently changes the machine trust store.
...
On September 11, 2026, WiX Toolset 3.14.1.8722 generated both
`UsbDk_99.99.90_ARM64.msi` and `UsbDk_Debug_99.99.90_ARM64.msi` without ICE or
...
This remains build and installer-package evidence only. Runtime installation
requires either test signing in a disposable VM or a trusted production
signature. USB device enumeration, exclusive access, transfer coverage,
rollback, and uninstall cleanup still require separate validation.
...
The `Release` ARM64 driver is signed with the WDK test certificate generated by
...
running the controller. `Debug_NoSign` is intentionally unsigned and is useful
only for build/debug inspection. To leave test mode, run
